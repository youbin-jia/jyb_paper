...
class FIDScoreTask(eval_task.EvalTask):
  ...
  def run_after_session(self, fake_dset, real_dset):
    ...
    with tf.Graph().as_default():
      fake_activations = tf.convert_to_tensor(value=fake_dset.activations)
      real_activations = tf.convert_to_tensor(value=real_dset.activations)
...
def compute_fid_from_activations(fake_activations, real_activations):
  ...
  with tf.compat.v1.Session(graph=tf.Graph()) as sess:
    fake_activations = tf.convert_to_tensor(value=fake_activations)
