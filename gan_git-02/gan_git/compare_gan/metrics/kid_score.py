...
def kid(fake_activations,
  ...
  def get_kid_batch(i):
    ...
    return (
        -2 * tf.reduce_mean(input_tensor=k_rg) + (tf.reduce_sum(input_tensor=k_rr) - tf.linalg.trace(k_rr)) /
        (m * (m - 1)) + (tf.reduce_sum(input_tensor=k_gg) - tf.linalg.trace(k_gg)) / (n * (n - 1)))
  ...
  if return_stderr:
    if n_bins < 5:
      return tf.reduce_mean(input_tensor=ests), np.nan
    mn, var = tf.nn.moments(x=ests, axes=[0])
    return mn, tf.sqrt(var / n_bins)
  else:
    return tf.reduce_mean(input_tensor=ests)
