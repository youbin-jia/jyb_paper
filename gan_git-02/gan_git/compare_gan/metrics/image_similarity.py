...
def verify_compatible_shapes(img1, img2):
  ...
  checks = []
  checks.append(tf.Assert(tf.greater_equal(tf.size(input=shape1), 3),
                          [shape1, shape2], summarize=10))
  checks.append(tf.Assert(tf.reduce_all(input_tensor=tf.equal(shape1[-3:], shape2[-3:])),
                          [shape1, shape2], summarize=10))
...
def f_special_gauss(size, sigma):
  ...
  size = tf.convert_to_tensor(value=size, dtype=tf.int32)
  sigma = tf.convert_to_tensor(value=sigma)
...
def _ssim_index_per_channel(
  ...
  filter_size = tf.reduce_min(
      input_tensor=tf.concat([tf.expand_dims(filter_size, axis=0),
                 shape1[-3:-1],
                 shape2[-3:-1]],
                axis=0))
  ...
  def reducer(x):  # pylint: disable=invalid-name
    ...
    y = tf.nn.depthwise_conv2d(input=x, filter=kernel, strides=[1] * 4, padding='VALID')
    return tf.reshape(y, tf.concat([shape[:-3], tf.shape(input=y)[1:]], 0))

  luminance, cs = _ssim_helper(img1, img2, reducer, max_val, compensation)

  # Average over the second and the third from the last: height, width.
  axes = tf.constant([-3, -2], dtype=tf.int32)
  ssim = tf.reduce_mean(input_tensor=luminance * cs, axis=axes)
  cs = tf.reduce_mean(input_tensor=cs, axis=axes)
  return ssim, cs
...
def _multiscale_ssim_helper(
  ...
  with tf.name_scope(name='MS-SSIM'):
    ...
    def do_pad(images, remainder):  # pylint: disable=invalid-name
      ...
      padding = tf.pad(tensor=padding, paddings=[[1, 0], [1, 0]])
      return [tf.pad(tensor=x, paddings=padding, mode='SYMMETRIC') for x in images]
    ...
    for k in range(len(power_factors)):
      with tf.name_scope(name='Scale%d' % k):
        if k > 0:
          ...
          need_padding = tf.reduce_any(input_tensor=tf.not_equal(remainder, 0))
          # pylint: disable=cell-var-from-loop
          padded = tf.cond(pred=need_padding,
                           true_fn=lambda: do_pad(flat_imgs, remainder),
                           false_fn=lambda: flat_imgs)
    ...
    ms_ssim = tf.reduce_prod(input_tensor=tf.pow(mcs_and_ssim, power_factors), axis=[-1])

    ms_ssim = tf.reduce_mean(input_tensor=ms_ssim, axis=[-1])  # Average over color channels.
    return ms_ssim
