...
def dragan_penalty(discriminator, x, y, is_training):
  ...
  with tf.compat.v1.name_scope("dragan_penalty"):
    _, var = tf.nn.moments(x=x, axes=list(range(len(x.get_shape()))))
    std = tf.sqrt(var)
    ...
    gradients = tf.gradients(ys=logits, xs=[x_noisy])[0]
    slopes = tf.sqrt(0.0001 + tf.reduce_sum(
        input_tensor=tf.square(gradients), axis=[1, 2, 3]))
    gradient_penalty = tf.reduce_mean(input_tensor=tf.square(slopes - 1.0))
...
def wgangp_penalty(discriminator, x, x_fake, y, is_training):
  ...
  with tf.compat.v1.name_scope("wgangp_penalty"):
    alpha = ops.random_uniform(shape=[x.shape[0], 1, 1, 1], name="alpha")
    interpolates = x + alpha * (x_fake - x)
    logits = discriminator(
        interpolates, y=y, is_training=is_training, reuse=True)[1]
    gradients = tf.gradients(ys=logits, xs=[interpolates])[0]
    slopes = tf.sqrt(0.0001 + tf.reduce_sum(
        input_tensor=tf.square(gradients), axis=[1, 2, 3]))
    gradient_penalty = tf.reduce_mean(input_tensor=tf.square(slopes - 1.0))
    return gradient_penalty
...
def l2_penalty(discriminator):
  ...
  with tf.compat.v1.name_scope("l2_penalty"):
    ...
    return tf.reduce_mean(
        input_tensor=[tf.nn.l2_loss(i) for i in d_weights], name="l2_penalty")
