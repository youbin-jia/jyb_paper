...
class SSGAN(modular_gan.ModularGAN):
  ...
  def discriminator_with_rotation_head(self, x, y, is_training):
    ...
    with tf.compat.v1.variable_scope("discriminator_rotation", reuse=tf.compat.v1.AUTO_REUSE):
      rotation_scores = linear(tf.reshape(final, (tf.shape(input=x)[0], -1)),
                               NUM_ROTATIONS,
  ...
  def create_loss(self, features, labels, params, is_training=True):
    ...
    if "rotation" in self._self_supervision:
      ...
      preds_onreal = tf.cast(tf.argmax(input=c_real_logits, axis=-1), rotate_labels.dtype)
      accuracy = tf.reduce_mean(
          input_tensor=tf.cast(tf.equal(rotate_labels, preds_onreal), tf.float32))
      c_real_probs = tf.nn.softmax(c_real_logits)
      c_fake_probs = tf.nn.softmax(c_fake_logits)
      c_real_loss = - tf.reduce_mean(
          input_tensor=tf.reduce_sum(input_tensor=rotate_labels_onehot * tf.math.log(c_real_probs + 1e-10), axis=1))
      c_fake_loss = - tf.reduce_mean(
          input_tensor=tf.reduce_sum(input_tensor=rotate_labels_onehot * tf.math.log(c_fake_probs + 1e-10), axis=1))
