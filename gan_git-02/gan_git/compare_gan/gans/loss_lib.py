...
def wasserstein(d_real_logits, d_fake_logits, d_real=None, d_fake=None):
  ...
  with tf.compat.v1.name_scope("wasserstein_loss"):
    check_dimensions(d_real, d_fake, d_real_logits, d_fake_logits)
    d_loss_real = -tf.reduce_mean(input_tensor=d_real_logits)
    d_loss_fake = tf.reduce_mean(input_tensor=d_fake_logits)
    d_loss = d_loss_real + d_loss_fake
    g_loss = -d_loss_fake
    return d_loss, d_loss_real, d_loss_fake, g_loss
...
def least_squares(d_real, d_fake, d_real_logits=None, d_fake_logits=None):
  ...
  with tf.compat.v1.name_scope("least_square_loss"):
    check_dimensions(d_real, d_fake, d_real_logits, d_fake_logits)
    d_loss_real = tf.reduce_mean(input_tensor=tf.square(d_real - 1.0))
    d_loss_fake = tf.reduce_mean(input_tensor=tf.square(d_fake))
    d_loss = 0.5 * (d_loss_real + d_loss_fake)
    g_loss = 0.5 * tf.reduce_mean(input_tensor=tf.square(d_fake - 1.0))
    return d_loss, d_loss_real, d_loss_fake, g_loss
...
def hinge(d_real_logits, d_fake_logits, d_real=None, d_fake=None):
  ...
  with tf.compat.v1.name_scope("hinge_loss"):
    check_dimensions(d_real, d_fake, d_real_logits, d_fake_logits)
    d_loss_real = tf.reduce_mean(input_tensor=tf.nn.relu(1.0 - d_real_logits))
    d_loss_fake = tf.reduce_mean(input_tensor=tf.nn.relu(1.0 + d_fake_logits))
    d_loss = d_loss_real + d_loss_fake
    g_loss = - tf.reduce_mean(input_tensor=d_fake_logits)
    return d_loss, d_loss_real, d_loss_fake, g_loss
