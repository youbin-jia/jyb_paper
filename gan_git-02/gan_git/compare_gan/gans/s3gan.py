...
class S3GAN(modular_gan.ModularGAN):
  ...
  def discriminator_with_additonal_heads(self, x, y, is_training):
    ...
    """
    print('--------------------------------------------------------------------s3gan.py/discriminator_with_additonal_heads,s')
    ...
    is_label_available = tf.cast(tf.cast(
        tf.reduce_sum(input_tensor=y, axis=1, keepdims=True), tf.float32) > 0.5, tf.float32)
    ...
    if not self._project_y:
      print('not self._project_y---------------------------------------------------s3gan.py/discriminator_with_additonal_heads,e')
    ...
    class_embedding = self.get_class_embedding(
        y=y, embedding_dim=x_rep.shape[-1], use_sn=use_sn)
    d_logits += tf.reduce_sum(input_tensor=class_embedding * x_rep, axis=1, keepdims=True)
    d_probs = tf.nn.sigmoid(d_logits)
    print('---------------------------------------------------s3gan.py/discriminator_with_additonal_heads,e')
    return d_probs, d_logits, rotation_logits, aux_logits, is_label_available
  #feature F
  def get_class_embedding(self, y, embedding_dim, use_sn):
    print('---------------------------------------------------s3gan.py/get_class_embedding,s')
    with tf.compat.v1.variable_scope("discriminator_projection", reuse=tf.compat.v1.AUTO_REUSE):
      ...
      logging.info("[Discriminator] embedded_y for projection: %s",
                   embedded_y.shape)
      print('---------------------------------------------------s3gan.py/get_class_embedding,e')
      return embedded_y
  ...
  def merge_with_rotation_data(self, real, fake, real_labels, fake_labels,
    ...
    print('---------------------------------------------------s3gan.py/merge_with_rotation_data,s')
    ...
    print('---------------------------------------------------s3gan.py/merge_with_rotation_data,e')
  ...
  def create_loss(self, features, labels, params, is_training=True):
    ...
    #new add
    self.logits = logits_real
    self.predicted_classes = tf.argmax(logits_real, 1)
    ...
    if self._self_supervision == "rotation":
      ...
      real_loss = -tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=rot_onehot * rot_real_logp, axis=1))
      fake_loss = -tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=rot_onehot * rot_fake_logp, axis=1))
    ...
    if self._use_predictor:
      ...
      self._tpu_summary.scalar("label_frac", tf.reduce_mean(input_tensor=is_label_available))
    print('---------------------------------------------------s3gan.py/create_loss,e')
