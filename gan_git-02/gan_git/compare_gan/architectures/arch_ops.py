...
def layer_norm(input_, is_training, scope):
  return tf.keras.layers.LayerNormalization(
      input_, trainable=is_training, scale=scope)
  ...
  for _ in range(power_iteration_rounds):
    if singular_value == "left":
      ...
      v = tf.math.l2_normalize(
          tf.matmul(tf.transpose(a=w), u), axis=None, epsilon=epsilon)
...
def conv2d(inputs, output_dim, k_h, k_w, d_h, d_w, stddev=0.02, name="conv2d",
  ...
  with tf.compat.v1.variable_scope(name):
    w = tf.compat.v1.get_variable(
        "kernel", [k_h, k_w, inputs.shape[-1], output_dim],
        initializer=weight_initializer(stddev=stddev))
    if use_sn:
      w = spectral_norm(w)
    outputs = tf.nn.conv2d(input=inputs, filters=w, strides=[1, d_h, d_w, 1], padding="SAME")
    if use_bias:
      bias = tf.compat.v1.get_variable(
...
def deconv2d(inputs, output_shape, k_h, k_w, d_h, d_w,
  ...
  with tf.compat.v1.variable_scope(name):
    ...
    return tf.reshape(tf.nn.bias_add(deconv, bias), tf.shape(input=deconv))
...
def weight_norm_linear(input_, output_size,
  ...
  with tf.compat.v1.variable_scope(name):
    if init:
      ...
      m_init, v_init = tf.nn.moments(x=x_init, axes=[0])
      ...
      scaler = g / tf.sqrt(tf.reduce_sum(input_tensor=tf.square(v), axis=[0]))
...
def weight_norm_conv2d(input_, output_dim,
  ...
  with tf.compat.v1.variable_scope(name):
    if init:
      ...
      x_init = tf.nn.conv2d(input=input_, filters=v_norm, strides=[1, d_h, d_w, 1],
                            padding="SAME")
      m_init, v_init = tf.nn.moments(x=x_init, axes=[0, 1, 2])
      ...
      w = tf.reshape(g, [1, 1, 1, output_dim]) * tf.nn.l2_normalize(
          v, [0, 1, 2])
      x = tf.nn.bias_add(
          tf.nn.conv2d(input=input_, filters=w, strides=[1, d_h, d_w, 1], padding="SAME"), b)
      return x
