...
class BigGanDeepResNetBlock(object):
  ...
  def _shortcut(self, inputs):
    ...
    with tf.compat.v1.variable_scope("shortcut", values=[inputs]):
      ...
      if self._scale == "down":
        shortcut = tf.nn.pool(input=shortcut, window_shape=[2, 2], pooling_type="AVG", padding="SAME",
  ...
  def apply(self, inputs, z, y, is_training):
    ...
    if inputs.shape[-1] != self._in_channels:
      raise ValueError(
          "Unexpected number of input channels (expected {}, got {}).".format(
              self._in_channels, inputs.shape[-1]))
    ...
    with tf.compat.v1.variable_scope(self._name, values=[inputs]):
      ...
      with tf.compat.v1.variable_scope("conv4", values=[outputs]):
        ...
        outputs = tf.nn.relu(outputs)
        if self._scale == "down":
          outputs = tf.nn.pool(input=outputs, window_shape=[2, 2], pooling_type="AVG", padding="SAME", strides=[2, 2],
                               name="avg_pool")
...
class Generator(abstract_arch.AbstractGenerator):
  ...
  def apply(self, z, y, is_training):
    ...
    for block_idx in range(num_blocks):
      ...
      if scale == "up" and net.shape[1] == 64:
...
class Discriminator(abstract_arch.AbstractDiscriminator):
  ...
  def apply(self, x, y, is_training):
    ...
    in_channels, out_channels = self._get_in_out_channels(
        colors=x.shape[-1], resolution=x.shape[1])
    num_blocks = len(in_channels)
    ...
    for block_idx in range(num_blocks):
      ...
      # At resolution 64x64 there is a self-attention block.
      if scale == "none" and net.shape[1] == 64:
    ...
    net = tf.nn.relu(net)
    h = tf.math.reduce_sum(input_tensor=net, axis=[1, 2])
    ...
    if self._project_y:
      ...
      with tf.compat.v1.variable_scope("embedding_fc"):
        ...
        logging.info("[Discriminator] embedded_y for projection: %s",
                     embedded_y.shape)
        out_logit += tf.reduce_sum(input_tensor=embedded_y * h, axis=1, keepdims=True)
