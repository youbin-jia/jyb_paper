...
class ImageDatasetV2(object):
  ...
  def __init__(self,
    ...
    self._local_train_ds_dir="/content/drive/My Drive/drive/gan/train"
    self._local_test_ds_dir="/content/drive/My Drive/drive/gan/test"
  @property
  def name(self):
    """Name of the dataset."""
    return self._name
  ...
  def _make_fake_dataset(self, split):
    ...
    print("--------------------------------------------datasets.py/_make_fake_dataset,s")
    ...
    print("--------------------------------------------datasets.py/_make_fake_dataset,e")
  ...
  def _replace_labels(self, split, ds, file_pattern=None):
    ...
    """
    print('-----------------------------------------------------datasets.py/_replace_labels,s')
    if not file_pattern:
      print('-------------------if not file_pattern:---------------_replace_labels,e')
      return ds
    ...
    print('-------------------------------------------------datasets.py/_replace_labels,e            file_pattern=',file_pattern)
    return ds
  ...
  def _replace_label(self, feature_dict, new_unparsed_label):
    ...
    """
    print('-----------------------------------------------------datasets.py/_replace_label,s')
    label_spec = {
        "file_name": tf.io.FixedLenFeature((), tf.string),
        "label": tf.io.FixedLenFeature((), tf.int64),
    }
    parsed_label = tf.io.parse_single_example(serialized=new_unparsed_label, features=label_spec)
    ...
    print('-----------------------------------------------------datasets.py/_replace_label,e')
    return feature_dict

  def _parse_fn(self, features):
    print('-----------------------------------------------------datasets.py/_parse_fn,s')
    image = tf.cast(features["image"], tf.float32) / 255.0
    print('-----------------------------------------------------datasets.py/_parse_fn,e')
  ...
  def _load_dataset(self, split):
    ...
    """
    print("--------------------------------------------datasets.py/_load_dataset,s")
    #if FLAGS.data_fake_dataset:
    #  return self._make_fake_dataset(split)
    if split == self._train_split:
        print("!!!!!!!!!!!!!!!!!!!!!!----load_local_datasets,  _local_train_ds_dir=",self._local_train_ds_dir)
        ds = self.get_local_datasets(self._local_train_ds_dir)
        return ds
    if split == self._eval_split:
        print("!!!!!!!!!!!!!!!!!!!!!!----load_local_datasets,  _local_test_ds_dir=",self._local_test_ds_dir)
        ds = self.get_local_datasets(self._local_test_ds_dir)
        return ds
    else:
        print('tfds.load()  ,!!!!!!!!!!!!!!!!!!!!!!!!!,   split=',split,',tfds.Split.TEST=',tfds.Split.TEST)
        ds = tfds.load(self._tfds_name, split=split, data_dir=FLAGS.tfds_data_dir, as_dataset_kwargs={"shuffle_files": False})
    print("------------------------_replace_labels")
    ds = self._replace_labels(split, ds)
    print("------------------------ds.map(self._parse_fn)")
    ds = ds.map(self._parse_fn)
    print("--------------------------------------------datasets.py/_load_dataset,e")
...
############################################################################################################
  ...
  def train_input_fn(self, params=None, preprocess_fn=None):
    ...
    """
    print("--------------------------------------------datasets.py/train_input_fn,s")
    #set seed
    if params is None:
      params = {}
    ...
    print("--------------------------------------------datasets.py/train_input_fn,e")
    #add buffer
    return ds.prefetch(tf.data.experimental.AUTOTUNE)
############################################################################################################
    ########################################################################################################
    ########################################################################################################
  def eval_input_fn(self, params=None, preprocess_fn=None, split=None, mode=tf.estimator.ModeKeys.EVAL):
    """Input function for reading data.
    ...
    Returns:
      `tf.data.Dataset` with preprocessed and batched examples.
    """
    print("--------------------------------------------datasets.py/eval_input_fn,s")
    ...
    print('--------befor self._load_dataset------------------split =',split,'self._eval_split=',self._eval_split)
    ds = self._load_dataset(split=self._eval_split)
    # No filter, no rpeat.
    ...
    if "batch_size" in params:
      ds = ds.batch(params["batch_size"], drop_remainder=True)
    print("--------------------------------------------datasets.py/eval_input_fn,e")
    return ds.prefetch(tf.data.experimental.AUTOTUNE)

  def preprocess_image(self, image):
    image = tf.image.decode_jpeg(image, channels=3)
    image = tf.image.resize(image, [self._resolution, self._resolution])
  ...
  def get_local_datasets(self, path_dir):
    print('-----------------------------------------get_local_datasets,s')
    ...
    print('-----------------------------------------get_local_datasets,e')
    return image_label_ds
###########################################################################################################
    
  # For backwards compatibility ImageDataset.
  def input_fn(self, params, mode=tf.estimator.ModeKeys.TRAIN, preprocess_fn=None):
    assert(mode == tf.estimator.ModeKeys.TRAIN)
    print("--------------------------------------------datasets.py/input_fn,s/e")
...
class RotorWindingDataset(ImageDatasetV2):
  """RotorWinding2020 as defined by TF Datasets."""

  def __init__(self, resolution, seed, filter_unlabeled=False):
    super(RotorWindingDataset, self).__init__(
        name="RotorWinding_{}".format(resolution),
        tfds_name="RotorWinding2020",
        resolution=resolution,
        colors=3,
        num_classes=2,
        eval_test_samples=1742,
        seed=seed)
    self._eval_split = tfds.Split.VALIDATION
    self._filter_unlabeled = filter_unlabeled
...
class SizeFilteredImagenetDataset(ImagenetDataset):
  ...
  def _train_filter_fn(self, image, label):
    ...
    del label
    size = tf.math.reduce_min(input_tensor=tf.shape(input=image)[:2])
...
DATASETS = {
    ...
    "mnist": MnistDataset,
    "RotorWinding_128": functools.partial(RotorWindingDataset, resolution=128),
